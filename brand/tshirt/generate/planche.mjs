...
import { GARMENT, TEE, POSE, NECK, HEM, LEG, img, poser } from './silhouette.mjs'
...
${T(20, 24, `Cotes en mm sous l'encolure, sur un Creator 2.0 taille L (${GARMENT.widthMm} x ${GARMENT.lengthMm} mm). Silhouette et motifs a la meme echelle.`, { s: 4.5, f: '#8a8a8a' })}
${panel('DOS', 20, `
...
${panel('DEVANT (vu de face)', 220, `
  ...
  ${T((100 + cCx) / 2, cTop + cH + 11, `${POSE.coeurAxeMm} mm`, { a: 'middle' })}
  <line x1="${cCx + cW / 2 + 4}" y1="${NECK}" x2="${cCx + cW / 2 + 4}" y2="${cTop + cH / 2}" stroke="#00AAEF" stroke-width="0.4"/>
  ${T(cCx + cW / 2 + 6, cTop + cH / 2 + 1, `${POSE.coeurMm} mm sous`, {})}
  ${T(cCx + cW / 2 + 6, cTop + cH / 2 + 6, "l'encolure, au centre", {})}
